...
final class FileIndexingTests: XCTestCase {
    ...
    func testIndexFile() {
        let tempFile = FileHelper.TemporaryFile().url
        guard let indexer = SearchIndexer.File.Create(fileURL: tempFile) else {
            XCTFail()
            return
        }
