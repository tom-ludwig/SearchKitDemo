...
struct ContentView: View {
    ...
    @State var displayedResults: [SearchIndex.SearchResult]?
    @State var indexer: SearchIndex?
    var body: some View {
        VStack {
            if let tempDoc = tempDoc {
                Text("\(tempDoc.fileURL)")
            }
            if let res = displayedResults {
                Text("\(res.description)")
            }
            HStack {
                Button("Search") {
                    displayedResults = loadFileNames()
