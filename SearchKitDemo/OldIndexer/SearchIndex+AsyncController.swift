//
//  SearchIndex+AsyncController.swift
//  SearchKitDemo
//
...
@objc public protocol DFSearchIndexAsyncControllerProtocol {
    ...
    func queueDidEmpty(_ indexer: SearchIndex.AsyncController)
    /// The queue's operation count did change
    func queueDidChange(_ indexer: SearchIndex.AsyncController, count: Int)
}
...
extension SearchIndex {
    ...
    @objc(DFSearchIndexAsyncController) public class AsyncController: NSObject {
        ...
        /// Initializer
        public init(index: SearchIndex, delegate: DFSearchIndexAsyncControllerProtocol?) {
            self.index = index
...
@objc(DFSearchIndexAsyncController) public extension SearchIndex.AsyncController {
    ...
    class SearchTask: NSObject {
        fileprivate let search: SearchIndex.ProgressiveSearch

        /// The search term used to create the task
        public let query: String

        /// Create a search task in the specified index
        fileprivate init(_ index: SearchIndex, query: String) {
            self.query = query
        ...
        public func next(
            ...
            complete: @escaping (SearchTask, SearchIndex.ProgressiveSearch.Results) -> Void
        ) {
            DispatchQueue.global(qos: .userInitiated).async {
                let results = self.search.next(maxResults, timeout: timeout)
                let searchResults = SearchIndex.ProgressiveSearch.Results(moreResultsAvailable: results.moreResultsAvailable, results: results.results)
